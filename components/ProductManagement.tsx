...
const APP_PRODUCTS: Product[] = [
  ...
  { 
    id: 'a2', 
    name: '耕地保护监测平台', 
    desc: '利用卫星遥感技术实现对耕地“非农化”、“非粮化”行为的动态监测，形成“天上看、地上查、网上管”的监管环。', 
...
export const ProductManagement: React.FC<Props> = ({ title = '产品管理' }) => {
  ...
  const [isOfflineModifyModalOpen, setIsOfflineModifyModalOpen] = useState(false);
  ...
  const handleOfflineModify = () => {
    if (productToModify) {
      setProducts(prev => prev.filter(p => p.id !== productToModify.id));
      setIsOfflineModifyModalOpen(false);
  ...
  if (!isData && !isTool && !isApp && !isMap) {
    return (
      <div className="p-8 bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="text-gray-500 font-medium">该模块正在建设中。</p>
      </div>
    );
  }
    ...
    <div className="flex gap-8 h-full min-h-[calc(100vh-12rem)]">
      ...
      <div className="w-64 bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-4 flex flex-col shrink-0">
        <div className="px-4 py-4 mb-4 border-b border-gray-50">
          <p className="text-[10px] text-gray-400 font-bold mt-1 uppercase tracking-widest">{title}资源目录</p>
        </div>
              ...
              {filteredProducts.map(product => (
                ...
                >
                  ...
                  <div className="relative h-60 overflow-hidden">
                    ...
                    {/* 类目标签 */}
                    <div className="absolute top-6 left-6 px-4 py-2 bg-white/20 backdrop-blur-md rounded-2xl border border-white/30">
                  ...
                  <div className="p-8 flex flex-col flex-1">
                    ...
                    <div className={`mt-auto flex items-center justify-between border-t border-gray-50 pt-6`}>
                      <div className="flex-1">
                        {/* 左侧“下线修改”按钮 */}
                        {activeTab === 'unlisted' && (isData || isTool || isApp || isMap) && (
                          <div className="relative group/tooltip inline-block">
                            <button 
                              onClick={() => { setProductToModify(product); setIsOfflineModifyModalOpen(true); }}
                              className="flex items-center text-xs font-black text-gray-400 hover:text-blue-600 transition-all active:scale-95 group/modify"
                            >
                              <Edit3 className="w-4 h-4 mr-2" />
                              下线修改
                            </button>
                            {/* Tooltip */}
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 px-3 py-2 bg-white border border-gray-100 rounded-xl shadow-xl opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-30">
                              <span className="text-[10px] font-black text-amber-600">退回资源管理修改</span>
                        ...
                        <div className="relative group/tooltip inline-block">
                          <button 
                            ...
                            className={`inline-flex items-center px-6 py-3 rounded-2xl text-[11px] font-black transition-all ${
                              product.isListed 
                                ? 'bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white hover:shadow-rose-500/25 shadow-lg' 
              ...
              <div className="relative mb-10">
                ...
                <div className="relative z-10 p-12 bg-white/40 backdrop-blur-3xl rounded-[4rem] border border-white shadow-2xl">
                  <img 
                    ...
                    alt="Empty box" 
                    className="w-64 h-64 opacity-70 grayscale"
                  />
                </div>
              </div>
              <h3 className="text-2xl font-black text-gray-900">暂无相关产品</h3>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-12 text-center">
              <div className="w-24 h-24 bg-[#fffbeb] text-[#f59e0b] rounded-full flex items-center justify-center mx-auto mb-8 shadow-sm">
                <div className="w-20 h-20 bg-[#fffbeb] rounded-full flex items-center justify-center border-4 border-white shadow-inner">
                   <AlertTriangle className="w-10 h-10" />
                </div>
              </div>
              <h3 className="text-3xl font-black text-[#1a237e] mb-5 tracking-tight">下线修改</h3>
              <div className="space-y-2">
                <p className="text-sm text-gray-500 font-bold leading-relaxed px-6">
                  确定返回到资源池进行修改吗？
                </p>
                <p className="text-sm text-gray-500 font-bold leading-relaxed px-6">
                  此操作将移除该产品条目，您需要在资源管理中重新提交。
                </p>
              </div>
            </div>
            
            <div className="px-10 pb-10 flex items-center space-x-5">
              <button 
                onClick={() => setIsOfflineModifyModalOpen(false)}
                className="flex-1 py-5 bg-white border-2 border-gray-100 text-gray-400 rounded-3xl text-base font-black hover:bg-gray-50 transition-all active:scale-95 shadow-sm"
              >
                取消
              </button>
              <button 
                onClick={handleOfflineModify}
                className="flex-1 py-5 bg-[#2563eb] text-white rounded-3xl text-base font-black hover:bg-blue-700 transition-all active:scale-95 shadow-xl shadow-blue-500/20"
              >
                确认下线
              </button>
            </div>
      ...
      {isAddCategoryModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md flex flex-col overflow-hidden">
            <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-lg font-black text-gray-900 tracking-tight">新增类目</h3>
            ...
            <div className="p-8 space-y-6">
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-2">类目名称</label>
                <input 
                  type="text" 
                  value={newCategoryName}
                  onChange={(e) => setNewCategoryName(e.target.value)}
                  placeholder="请输入新类目名称..." 
                  className="w-full px-5 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:bg-white text-sm font-bold"
                  autoFocus
                />
              </div>
            </div>
            <div className="px-8 py-6 border-t border-gray-50 flex justify-end space-x-3">
              <button 
                onClick={() => setIsAddCategoryModalOpen(false)}
                className="px-6 py-2.5 bg-white border border-gray-200 text-gray-500 rounded-xl text-sm font-bold hover:bg-gray-100"
              >
                取消
              </button>
              <button 
                ...
                className="px-8 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-bold hover:bg-blue-700 shadow-xl shadow-blue-500/20"
              >
                确认添加
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md flex flex-col overflow-hidden">
            <div className="p-10 text-center">
              ...
              <h3 className="text-2xl font-black text-gray-900 mb-2">删除类目</h3>
              <p className="text-sm text-gray-500 font-medium px-4">
                确定要删除“<span className="text-gray-900 font-black">{categoryToDelete?.name}</span>”类目吗？
              </p>
            </div>
            <div className="px-10 py-8 bg-gray-50 border-t border-gray-100 flex items-center space-x-4">
              <button onClick={() => setIsDeleteModalOpen(false)} className="flex-1 py-4 bg-white border border-gray-200 text-gray-500 rounded-2xl text-sm font-black">取消</button>
              <button onClick={executeDeleteCategory} className="flex-1 py-4 bg-rose-500 text-white rounded-2xl text-sm font-black">确认删除</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md flex flex-col overflow-hidden">
            <div className="p-10 text-center">
              ...
              <h3 className="text-2xl font-black text-gray-900 mb-2">无法删除</h3>
              <p className="text-sm text-gray-500 font-medium px-4">
                该类目下包含产品数据，无法删除！请先移除类目下的产品。
              </p>
            </div>
            <div className="px-10 py-8 bg-gray-50 border-t border-gray-100">
              <button onClick={() => setIsWarningModalOpen(false)} className="w-full py-4 bg-gray-900 text-white rounded-2xl text-sm font-black">我知道了</button>
            </div>
          </div>
        </div>
      )}
